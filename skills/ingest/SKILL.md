...
> Skip this step if running in silent/auto mode — i.e. called from `/alex-loop`
> or any context where the prompt says to skip discussion.
...
When called from `/alex-loop` or any headless/automated context, the ingest
workflow runs identically except:
- **Step 2 is skipped** — no discussion, no waiting for user input
- All new files in `raw/` are processed automatically
- Results are printed as a summary at the end instead of discussed interactively

## What's Next

After ingesting sources, the user can:
- **Ask questions** with `/alex-query` to explore what was ingested
- **Ingest more sources** — clip another article and run `/alex-ingest` again
- **Health-check** with `/alex-lint` after every 10 ingests to catch gaps
- **Automate ingestion** with `/alex-loop` — runs silently on a timed interval
