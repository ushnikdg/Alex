...
description: >
  ...
  the user wants to create a alex, initialize a vault, set up a
...
| Cursor | `cursor.md` | `alex.mdc` | `<vault>/.cursor/rules/` |
...
3. **How to start** — open the vault folder in Obsidian, clip an article to `raw/`, then run `/alex-ingest`
...
2. **Ingest sources** with `/alex-ingest` — processes raw files into wiki pages
3. **Ask questions** with `/alex-query` — searches and synthesizes from the wiki
4. **Health-check** with `/alex-lint` — run after every 10 ingests or monthly
